...
if (!logger) {
  var log_file = 'usgs_helpers'
  logger = require('../helpers/logger.js')(log_file)
}
...
module.exports = {
  ...
  get_datasetName: function (scene_id, acquisition_date) {
    ...
    const use_c1 = this.check_c1(acquisition_date)
    ...
    //  dataset name
    switch (true) {
      case (proudctAbbrevation === "LC8" ) && (use_c1): //LANDSAT 8
        return  "LANDSAT_8_C1"
        break
      case (proudctAbbrevation === "LC8" ) && (!use_c1): //LANDSAT 8
        return  "LANDSAT_8_C1" // used to be "LANDSAT_8"
        break
      case (proudctAbbrevation === "LE7") && (use_c1): //LANDSAT 7 with slc off
        return "LANDSAT_ETM_C1"
        break
      case (proudctAbbrevation === "LE7") && (slc_off) && (!use_c1): //LANDSAT 7 with slc off
        return "LANDSAT_ETM_C1" // used to be LANDSAT_ETM_SLC_OFF
        break
      case (proudctAbbrevation === "LE7") && (!slc_off) && (!use_c1): //LANDSAT 7 with slc on
        return "LANDSAT_ETM_C1" // used to be LANDSAT_ETM
        break
      case (proudctAbbrevation === "LT5") && (use_c1): //LANDSAT 5
        return "LANDSAT_TM_C1"
        break
      case (proudctAbbrevation === "LT4") && (use_c1): //LANDSAT 4
        return "LANDSAT_TM_C1"
        break
      case (proudctAbbrevation === "LT5") && (!use_c1): //LANDSAT 5
        return "LANDSAT_TM_C1" // used to be LANDSAT_TM
        break
      case (proudctAbbrevation === "LT4")  && (!use_c1): //LANDSAT 4
        return "LANDSAT_TM_C1" // used to be LANDSAT_TM
        break
  ...
  get_usgsapi_response: function(action, body, num_attempts) {
    ...
    return axios.post(
      request_action,
      request_body,
      USGS_CONSTANT.REQUEST_POST_HEADERS
    )
    .then( response =>  {
      ...
      if (response_data === null || response.data.error) {
        return Promise.reject(new Error(this.get_response_error(response)))
      }
      return response_data

    })
    .catch(function (error) {
      if (error.message.indexOf('Rate limit exceeded') >= 0) {
        if (num_attempts < MAX_REQUEST_ATTEMPTS) {
          logger.log(
            logger.LEVEL_INFO,
            'INFO USGS API call failed (simultaneous requests error): '
            + 'will try again until we reach the limit for request attempts '
          ...
          logger.log(
            logger.LEVEL_ERROR,
            'ERROR USGS API call failed (simultaneous requests error): '
            + 'limit for request attempts reached'
          )
          return Promise.reject(error)
        }
      }
      logger.log(logger.LEVEL_ERROR, error.stack)
      return Promise.reject(error)
    })
  },
