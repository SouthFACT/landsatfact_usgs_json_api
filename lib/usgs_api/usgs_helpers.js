...
module.exports = {
  ...
  sort_scene_records_by_dataset: function(records) {
    if (records && records.length) {
      ...
      USGS_CONSTANT.LANDSAT_DATASETS.forEach(function (dataset_name) {
    ...
    return axios.post(request_action , request_body_login, USGS_CONSTANT.REQUEST_POST_HEADERS)
      .then(function (response) {
        ...
        if (response_data == null){
          ...
          var error = _this.get_response_error(response);
        ...
        if (num_attempts < MAX_LOGIN_ATTEMPTS) {
          return _this.get_api_key(++num_attempts)
        }
      })
    ...
    return axios.post(request_action, request_body, USGS_CONSTANT.REQUEST_POST_HEADERS)
      ...
      .then( response =>  {
        ...
        if (response_data === null || response.data.error) {
          return Promise.reject(_this.get_response_error(response))
        }
        return response_data

      }).catch(function (error) {
        if (num_attempts < MAX_REQUEST_ATTEMPTS &&
            error.indexOf('Rate limit exceeded') >= 0) {
