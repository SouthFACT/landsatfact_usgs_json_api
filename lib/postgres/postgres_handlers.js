...
module.exports = {
    ...
    pool_query_db: function(pg_pool, query_text, parameters){
      ...
      pg_pool.connect(function(err, client, done) {
        if(err) {
          logger.log('error', 'error fetching client from pool: ' + err.message);
           console.error('error fetching client from pool', err.message);
           return
        }
        ...
        client.query(query_text, parameters, function(err, result) {
          ...
          if(err) {
            ...
            if(err.code === '23505'){
              ...
              //  console.log(err.detail  + ' In the table ' + err.table);
               return
