...
module.exports = {
  ...
  */
  ...
  update_metadata: function (field_set) {
    ...
    //when adding a new field you will also need to add field to the valid fields in is_valid_fieldset() function
    //limit to only fields we want to update - " scene_id, collection_category, browse_url, cc_full, data_type_l1, landsat_product_id
    var fields = [field_parameters[0], //scene_id 1
                  field_parameters[2], //collection_category 2
                  field_parameters[5], //browse_url 3
                  field_parameters[8], //cc_full 4
                  field_parameters[13], //data_type_l1 4
                  field_parameters[1] //landsat_product_id 5
                ]

    //update missing data
    const update_sql = ""
    +"UPDATE landsat_metadata"
      +" SET browse_url = $3::text, collection_category = $2::text, landsat_product_id =$6, cc_full = $4, data_type_l1 =  substr($5::text,position(' ' in $5::text)+1,5)"
  ...
  insert_metadata: function (field_set) {
    ...
    +"INSERT INTO landsat_metadata("
      +"scene_id, landsat_product_id, collection_category, sensor, acquisition_date, browse_url, path, row, cc_full, "
