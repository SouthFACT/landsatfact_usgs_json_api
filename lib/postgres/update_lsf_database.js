...
//creates an array of field names from the USGS api so we can compare for validity
 ...
 insert_metadata: function(field_set){
   ...
   const verify_sql = "DELETE FROM landsat_metadata WHERE scene_id in" +
                        	"(SELECT DISTINCT m.scene_id" +
                        	"FROM landsat_metadata AS m" +
                        	 "LEFT OUTER JOIN landsat_quads AS wrs ON" +
                        		"substr(m.scene_id, 4, 6) = wrs.wrs2_code" +
                        	"WHERE wrs.wrs2_code IS NULL)"

   PG_HANDLER.pool_query_db(pg_pool, insert_sql, field_parameters);
   PG_HANDLER.pool_query_db(pg_pool, verify_sql, field_parameters);
 ...
 metadata_to_db: function(field_set){
   ...
   if(valid){
     ...
     console.log('Inserting ' + field_set[0].name + ': ' + field_set[0].value + '.')
     logger.log('info', 'Inserting ' + field_set[0].name + ': ' + field_set[0].value + '.');
