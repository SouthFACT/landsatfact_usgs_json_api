...
if (!logger) {
  var log_file = 'postgres_handlers'
...
module.exports = {
    ...
    pg_pool: function(connection_obj){
      ...
      var pg_client = new pg.Pool(connection_obj)
        ...
        pg_client.on('error', function (err, client) {
          ...
          logger.log(logger.LEVEL_ERROR, "Client Error: %s", err.message)
    ...
    pool_query_db: function(pg_pool, query_text, query_params, callback) {
      ...
      pg_pool.connect(function(err, client, done) {
        if(err) {
          logger.log(logger.LEVEL_ERROR, 'error fetching client from pool: ' + err.message)
          return
        }
        ...
        client.query(query_text, query_params, function(err, result) {
          ...
          if(err) {
            ...
            if(err.code === '23505'){
              logger.log(logger.LEVEL_ERROR, 'error running query: ' + err.detail  + ' This happend while inserting data into the table ' + err.table + '.  The unique rule is defined by the constraint ' + err.constraint)
            //all other errors just use the default
            } else {
              logger.log(logger.LEVEL_ERROR, 'error running query: ' + err.message)
            }
          }

          //normal query completed successfully
          if (result) {
            logger.log(logger.LEVEL_INFO, 'Query completed successfully: for ' + result.command  + ' and returned ' + result.rowCount + ' row(s)' )
          } else {
            logger.log(logger.LEVEL_INFO, 'Query completed successfully')
          }
