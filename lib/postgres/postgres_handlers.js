    ...
    pool_query_db: function(pg_pool, query_text, query_params, callback) {
      ...
      pg_pool.connect(function(err, client, done) {
        ...
        client.query(query_text, query_params, function(err, result) {
          ...
          if (result) {
            logger.log(logger.LEVEL_ERROR, 'Query completed successfully: for ' + result.command  + ' and returned ' + result.rowCount + ' row(s)' )
          } else {
            logger.log(logger.LEVEL_ERROR, 'Query completed successfully')
          }
